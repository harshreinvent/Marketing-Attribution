...
import type { FunnelCampaignRow } from "@repo/types";
import { formatCurrency, formatNumber, formatPercent } from "@repo/utils";
import { ExportButton } from "@/components/common/ExportButton";

type Props = { rows: FunnelCampaignRow[] };

export function FunnelCampaignTable({ rows }: Props) {
  return (
    <div>
      <div className="flex justify-end mb-2">
        <ExportButton data={rows as unknown as Record<string, unknown>[]} filename="funnel-by-campaign" />
      </div>
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.campaign} className="border-b">
                <td className="py-2 pr-4 max-w-48 truncate">{row.campaign}</td>
