...
// ─── Color palette ────────────────────────────────────────────────────
const MAP_COLOR    = '#0ea5e9'   // sky blue   — Maps Impressions
const SEARCH_COLOR = '#f97316'   // orange     — Search Impressions

// Impression breakdown stacked bar colors
const IMP_COLORS = {
  desktopMaps:   '#1d4ed8',  // dark blue
  desktopSearch: '#0ea5e9',  // sky blue
  mobileMaps:    '#7c3aed',  // purple
  mobileSearch:  '#a78bfa',  // light purple
}

// ─── Section title styled like the screenshot ────────────────────────
...
// ─── Stacked bar chart for all 4 impression metrics per location ─────
const ImpressionStackedBar = ({ data }: {
  data: {
    location: string
    desktopMaps: number
  ...
  const chartData = data.map(d => ({
    location: d.location.length > 10 ? d.location.slice(0, 10) + '…' : d.location,
  ...
  return (
    <ResponsiveContainer width="100%" height={240}>
      <BarChart data={chartData} margin={{ top: 5, right: 10, left: -10, bottom: 0 }}>
        ...
        <Bar dataKey="Desktop Maps"   stackId="a" fill={IMP_COLORS.desktopMaps}   radius={[0,0,0,0]} />
        <Bar dataKey="Desktop Search" stackId="a" fill={IMP_COLORS.desktopSearch} radius={[0,0,0,0]} />
        <Bar dataKey="Mobile Maps"    stackId="a" fill={IMP_COLORS.mobileMaps}    radius={[0,0,0,0]} />
        <Bar dataKey="Mobile Search"  stackId="a" fill={IMP_COLORS.mobileSearch}  radius={[3,3,0,0]} />
      </BarChart>
    </ResponsiveContainer>
  )
}
...
export const GbpTab = ({ clientId, dateRange }: { clientId: string; dateRange: DateRange }) => {
  ...
  if (!data)     return null
  if (!data.hasIntegration) return <NoIntegration service="Google Business Profile" />
  ...
  // Bar chart data for Calls + Directions by Branch
  ...
  // Bar chart data for Profile Views — Maps vs Search
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <ExportButton data={data.locations} filename="gbp-location-performance" />
          </div>
        </CardHeader>
            ...
            <tbody>
              {pagedLocations.length === 0 ? (
                ...
                <tr key={i} className="hover:bg-slate-50 transition-colors border-b border-slate-50">
                  <Td className="text-slate-400 text-xs">{(locPage - 1) * LOC_PAGE_SIZE + i + 1}.</Td>
                  <Td className="font-medium text-slate-800">{row.name}</Td>
                  <Td className="font-semibold text-brand-600">{formatExact(row.callClicks)}</Td>
                  ...
                  <Td className="font-medium">{formatExact(row.totalEngagement)}</Td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {/* Calls + Directions by Branch */}
        <Card>
          <CardContent className="pt-4">
            <SectionTitle>Calls + Directions by Branch</SectionTitle>
            <ResponsiveContainer width="100%" height={240}>
              <BarChart data={callDirChartData} margin={{ top: 5, right: 10, left: -10, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
                <XAxis dataKey="location" tick={{ fontSize: 10, fill: '#94a3b8' }} />
                <YAxis tick={{ fontSize: 10, fill: '#94a3b8' }} />
                <Tooltip contentStyle={{ borderRadius: 8, border: '1px solid #f1f5f9', fontSize: 12 }} />
                <Legend wrapperStyle={{ fontSize: 11 }} />
                <Bar dataKey="Call Clicks"        fill="#3b82f6" radius={[3, 3, 0, 0]}
                  label={{ position: 'top', fontSize: 10, fill: '#1e40af' }} />
                <Bar dataKey="Direction Requests" fill="#f97316" radius={[3, 3, 0, 0]}
        ...
        <Card>
          <CardContent className="pt-4">
            <SectionTitle>Profile Views — Maps vs Search by Branch</SectionTitle>
            <ResponsiveContainer width="100%" height={240}>
              <BarChart data={profileViewsChartData} margin={{ top: 5, right: 10, left: -10, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
                <XAxis dataKey="location" tick={{ fontSize: 10, fill: '#94a3b8' }} />
                <YAxis tick={{ fontSize: 10, fill: '#94a3b8' }} />
                <Tooltip contentStyle={{ borderRadius: 8, border: '1px solid #f1f5f9', fontSize: 12 }} />
                <Legend wrapperStyle={{ fontSize: 11 }} />
                <Bar dataKey="Maps Impressions"   fill={MAP_COLOR}    radius={[3, 3, 0, 0]} />
                <Bar dataKey="Search Impressions" fill={SEARCH_COLOR} radius={[3, 3, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
      ...
      {/* ── Daily trend + Donut row ──────────────────────────────── */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {/* Daily GBP Activity Trend */}
        <Card>
          <CardContent className="pt-4">
            ...
            <ResponsiveContainer width="100%" height={240}>
              <LineChart data={data.dailyTrend} margin={{ top: 5, right: 15, left: -10, bottom: 0 }}>
                ...
                <Line type="monotone" dataKey="calls"         name="Calls"         stroke="#475569" strokeWidth={2} dot={false} />
                <Line type="monotone" dataKey="directions"    name="Directions"    stroke="#f97316" strokeWidth={2} dot={false} />
                <Line type="monotone" dataKey="websiteClicks" name="Website Clicks" stroke="#3b82f6" strokeWidth={2} dot={false} />
              </LineChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>

        {/* Impressions Breakdown stacked bar */}
        <Card>
          <CardContent className="pt-4">
